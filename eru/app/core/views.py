...
# !! DEPRECATED !!: Migrado para i18n do django
# --
# @version  1.0
# @since    02/10/2025
...
class EmpresaListView(LoginRequiredMixin, PermissionRequiredMixin, AjaxableListMixin, CSVExportMixin, BaseListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(object_list=filtro.qs, **kwargs)
        context.update({'filter': filtro, 'table': EmpresaTable(filtro.qs, request=self.request).config(self.request, filtro)})
        return context
...
class EmpresaUpdateView(LoginRequiredMixin, PermissionRequiredMixin, BaseUpdateView):
    model = Empresa
    form_class = EmpresaForm
...
class FilialDataView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        qs = request.user.profile.filiais.all()
        if empresa_id:
